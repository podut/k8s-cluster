...
def save_generated_secrets(secrets: dict, namespace: str):
    """Push generated secrets directly to Vault. Falls back to local .env if Vault unavailable."""
    if not secrets:
        return

    token = os.getenv("VAULT_ROOT_TOKEN")
    addr = os.getenv("VAULT_ADDR", "http://vault.vault.svc:8200")
    site = namespace.replace("wp-", "")

    if token:
        try:
            import requests as _req
            vault_path = f"{addr}/v1/secret/data/{site}"
            # Read existing keys first to avoid overwriting
            resp = _req.get(vault_path, headers={"X-Vault-Token": token}, timeout=5)
            existing = resp.json().get("data", {}).get("data", {}) if resp.status_code == 200 else {}
            existing.update(secrets)
            resp = _req.post(vault_path, headers={"X-Vault-Token": token},
                             json={"data": existing}, timeout=5)
            if resp.status_code in (200, 204):
                log.info("✅ Generated secrets saved to Vault at secret/%s", site)
                return
        except Exception as e:
            log.warning("Vault write failed, falling back to file: %s", e)

    # Fallback: write to file
    env_file = _data_dir / "secrets_to_vault.env"
    with open(env_file, "a") as f:
        f.write(f"\n# --- Secrets for {namespace} ---\n")
        for k, v in secrets.items():
            f.write(f"{k}={v}\n")
    log.warning("⚠ Secrets saved to %s — move to Vault manually", env_file)
